...
static NSString * cellitifter8 = @"cellitifter8";
static NSString * cellitifter9 = @"cellitifter9";


@interface SendApplyViewController ()<UITableViewDelegate, UITableViewDataSource,UITextFieldDelegate,UITextViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,PYPhotosViewDelegate,AdressListViewControllerDelegate>
{
...
@implementation SendApplyViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    [self.customNavBar setTitle:@"发起申请"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        if (adressModel) {
            SelectAdressTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellitifter4];
            
            if (!cell) {
                cell = [[SelectAdressTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellitifter4];
            }
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            static NSString * nullCell = @"nullCell";
            SelectAdressTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:nullCell];
            if (!cell) {
                cell = [[SelectAdressTableViewCell alloc] initWithNullAdressStyle:UITableViewCellStyleDefault reuseIdentifier:nullCell];
            }
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            return cell;
        }
    }else if (indexPath.row == 5){
        HelpSAdressTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellitifter5];
        if (!cell) {
            cell = [[HelpSAdressTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellitifter5];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else if (indexPath.row == 6){
        WeightTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellitifter6];
        if (!cell) {
            cell = [[WeightTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellitifter6];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
        
    }else if (indexPath.row == 7){
        ExpressFeeTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellitifter7];
        if (!cell) {
            cell = [[ExpressFeeTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellitifter7];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else if (indexPath.row == 8){
        SelectFreightTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellitifter8];
        if (!cell) {
            cell = [[SelectFreightTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellitifter8];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else{
        HelpProtocolTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellitifter9];
        if (!cell) {
            cell = [[HelpProtocolTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellitifter9];
        }
        cell.backgroundColor = kUIColorFromRGB(0xf5f5f5);
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
...
//    return;
    ...
    if (!fCell) {
        NSIndexPath * indexPtah = [NSIndexPath indexPathForRow:8 inSection:0];
    ...
    [AFNetworkingUtil uploadImageWithOperations:nil withImageArray:dateSourceArray withtargetWidth:SCREEN_WIDTH withUrlString:urlStr withSuccessBlock:^(NSDictionary *object) {
        ...
        NSDictionary * dic = @{@"accessToken":[UserModel getUser].accessToken,@"orderId":_orderID,@"applyTitle":_textTF.text,@"applyContent":_textView.text,@"backdropImg":bgImageCell.imageStr,@"backdropBottomImg":@"",@"willing":@(fCell.yesSelect),@"applyImgs":imageStr,@"mailAddress":adressString,@"mailName":adressModel.mailName,@"mailPhone":adressModel.mailPhone,@"province":adressModel.province,@"city":adressModel.city,@"county":adressModel.county};
        NSLog(@ "publish Paeament= %@",dic);
        [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        [DonateModel needTisDonateWithParament:dic success:^(NSDictionary *dic) {
            ...
            if ([dic[@"code"] isEqualToString:NormalStatus]) {
                [_delegate sendApllySuccessRefeshDonateDetailView];

...
+(instancetype)shareManager
{
    ...
    static AFNetworkingUtil * manager = nil;
...
//        self.requestSerializer.cachePolicy = NSURLRequestReturnCacheDataElseLoad;//有缓存就用缓存，没有缓存就重新请求
...
//        [self.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
        [self.requestSerializer setValue:[UserModel getUser].accessToken forHTTPHeaderField:@"NARAN-ACCESS-TOKEN"];
...
//        [self.requestSerializer setValue:apikey forHTTPHeaderField:@"apikey"];
        ...
        /**设置接受的类型*/
        [self.responseSerializer setAcceptableContentTypes:[NSSet setWithObjects:@"text/plain",@"application/json",@"text/json",@"text/javascript",@"text/html",@"multipart/form-data", nil]];
...
+(void)requestWithType:(HttpRequestType)type withUrlString:(NSString *)urlString withParaments:(id)paraments withSuccessBlock:(requestSuccess)successBlock withFailureBlock:(requestFailure)failureBlock progress:(downloadProgress)progress
{
    ...
    switch (type) {
        ...
        case HttpRequestTypeGet:
        {
            ...
            [[AFNetworkingUtil shareManager] GET:urlString parameters:paraments progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            
        {
            ...
            [[AFNetworkingUtil shareManager] POST:urlString parameters:paraments progress:^(NSProgress * _Nonnull uploadProgress) {
                ...
                successBlock(responseObject);
                //登录信息过期
                if ([responseObject[@"code"] isEqualToString:@"8888"]) {
                    
                    UIAlertController *alertController;
                    if (!alertController) {
                        alertController = [UIAlertController alertControllerWithTitle:@"用户登录信息过期" message:nil preferredStyle:UIAlertControllerStyleAlert];
                    ...
                    UIViewController *currentVC = [AFNetworkingUtil getCurrentVCFrom:rootViewController];
...
//            UIImage *  resizedImage =  [UIImage IMGCompressed:image targetWidth:width];
            ...
            //拼接data    multipart/form-data
            [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"picflie%ld",(long)i] fileName:@"image.png" mimeType:@" image/jpeg"];

//            [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"picflie%ld",(long)i] fileName:@"image.png" mimeType:@" multipart/form-data"];
...
+(void)cancelAllRequest
{
    ...
    [[AFNetworkingUtil shareManager].operationQueue cancelAllOperations];
...
+(void)cancelHttpRequestWithRequestType:(NSString *)requestType requestUrlString:(NSString *)string
{
    ...
    NSString * urlToPeCanced = [[[[AFNetworkingUtil shareManager].requestSerializer requestWithMethod:requestType URLString:string parameters:nil error:&error] URL] path];
    
    
    for (NSOperation * operation in [AFNetworkingUtil shareManager].operationQueue.operations) {

...
//
    ...
    [self getREcommendDta];

}
- (void)getREcommendDta
{
    NSDictionary * parament = @{@"pageNum":@1,@"pageSize":@10};
    [RecommendModel getRecommendDataWithParament:parament success:^(NSDictionary *dic) {
        NSLog(@"dic = %@",dic);
        NSArray * tempArray = [RecommendModel mj_objectArrayWithKeyValuesArray:dic[@"data"][@"contents"]];
        [self.recommendArray addObjectsFromArray:tempArray];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_recommendArray.count) {
        model = _recommendArray[indexPath.row];
    }
...
{
    RecommendModel * model = _recommendArray[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RecommendDetailViewController * recommentDetailVC = [[RecommendDetailViewController alloc] init];
    recommentDetailVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:recommentDetailVC animated:YES];
}

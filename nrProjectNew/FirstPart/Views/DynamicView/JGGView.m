    ...
    if (self) {
        for (NSUInteger i=0; i<dataSource.count; i++) {
            ...
            if ([dataSource[i] isKindOfClass:[UIImage class]]) {
                ...
                [iv sd_setImageWithURL:[NSURL URLWithString:dataSource[i]] placeholderImage:[UIImage imageNamed:@"6633.jpg_wh1200"]];
...
-(void)JGGView:(JGGView *)jggView DataSource:(NSArray *)dataSource completeBlock:(TapBlcok)tapBlock
{
        for (NSUInteger i=0; i<dataSource.count; i++) {
            ...
            if ([dataSource[i] isKindOfClass:[UIImage class]]) {
                ...
                [iv sd_setImageWithURL:[NSURL URLWithString:dataSource[i]] placeholderImage:[UIImage imageNamed:@"dynamicImage"]];
